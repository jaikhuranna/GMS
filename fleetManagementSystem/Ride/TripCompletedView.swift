...
//                  .onAppear {
//                    // now that navigation has happened, dismiss the checklist screen
//                    onDone()
//                  },
...
//                  showError = true
//                } else {
//                  // *only* trigger the NavigationLink
//                  navigateToDriverProfile = true
//                }
//              }
//            }
...
struct TripCompletedCard: View {
  let bookingRequestID: String
    let onHideOverlay: () -> Void  // now just hides overlay
  @ObservedObject var viewModel: AuthViewModel
  ...
  var body: some View {
    VStack(spacing: 16) {
      ...
      // Hidden link that only activates on success
        NavigationLink(
                destination:
                  DriverProfile(viewModel: viewModel)
                    .onAppear {
                      // as soon as DriverProfile is actually on screen, hide the overlay
                      onHideOverlay()
