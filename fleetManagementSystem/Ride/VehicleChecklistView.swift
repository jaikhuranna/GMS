            ...
            VStack(spacing: 0) {
                ...
                Button(action: {
                    if selectedItems.count == 6 {
                        navigateToMap = true // Trigger navigation when all items are selected
                    } else {
                        // Optionally show an alert or handle incomplete checklist
                    }
                }) {
                    HStack {
                ...
                NavigationLink(destination: NavigationMapView(from: "Start Location", to: "Destination Location"), isActive: $navigateToMap) {
                    EmptyView()
                }
                .hidden()
